...
| Reports show a compact plan indicator and mobile scroll guidance | `ProductionMonthlyOperationTable.styles.test.js`, report render tests | PASS |
...
The selected production order's existing `plannedQuantity` is passed to both the daily and monthly report components. The UI displays the plan, the allowed ±100 variance, and the first overage value in a compact information indicator. Totals above that limit receive the error color, stronger cell treatment, and a `Vượt kế hoạch` label. The report also provides mobile scroll guidance without changing the horizontal table model. No backend or report service changes are required.
