...
export function AttendancePage() {
  ...
  return (
    <div className="erp-feature-page erp-attendance-page">
      ...
      <div className="erp-attendance-filters"><label className="erp-field erp-attendance-filter-field"><span className="erp-field-label">Nhân viên</span><select className="erp-control" value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}><option value="">Tất cả nhân viên</option>{activeAttendanceEmployees(Object.values(cache.employeesById), monthKey).map((employee) => <option key={employee.employeeId} value={employee.employeeId}>{employee.employeeCode} — {employee.fullName}{employee.isActive === false ? " (Đã tắt)" : ""}</option>)}</select></label><span className="erp-field-hint">Nhân viên tắt trong tháng vẫn được xem ở chế độ chỉ đọc; từ tháng sau sẽ ẩn.</span></div>
