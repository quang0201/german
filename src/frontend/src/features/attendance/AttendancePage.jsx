...
export function AttendancePage() {
  ...
  return (
    <div className="erp-feature-page erp-attendance-page">
      ...
      <div className="erp-attendance-filters"><label className="erp-field erp-attendance-filter-field"><span className="erp-field-label">Nhân viên</span><select className="erp-control" value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}><option value="">Tất cả nhân viên</option>{activeAttendanceEmployees(Object.values(cache.employeesById)).map((employee) => <option key={employee.employeeId} value={employee.employeeId}>{employee.employeeCode} — {employee.fullName}</option>)}</select></label><span className="erp-field-hint">Nhân viên đã tắt không thể chọn chấm công mới · Ô trống = chưa nhập · nhập P/Ô cho nghỉ · TC nhập một lần theo ngày</span></div>
