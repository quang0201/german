...
export function ProductionMatrixBatchEntryDialog({ day, employees = [], onClose, onSaved }) {
  ...
  async function save() {
    ...
    if (attendanceOnly) {
      ...
      try {
        await api.put("/api/attendance/monthly", buildAttendanceMonthPayload({
          workDate: day.isoDate,
          employeeId,
          hourDraft,
        }));
        onSaved?.({ createdCount: 0, attendanceOnly: true });
      } catch (requestError) { setError(requestError.message || "Không thể lưu chấm công."); }
      ...
      <section className="erp-dialog erp-matrix-batch-dialog" role="dialog" aria-modal="true" aria-labelledby="matrix-batch-title">
        ...
        <div className="erp-dialog-body">
          ...
          {inputMode !== "direct" && <div className="erp-matrix-batch-hours" aria-label={attendanceOnly ? "Giờ chấm công" : "Giờ phân bổ"}>
            ...
            {(inputMode === "attendance-shifts" || attendanceOnly) && <>{hourDraft.shifts.map((shift) => <label key={shift.slotNumber}><span>{shift.shiftName}</span><input className="erp-control" type="number" min="0" step="any" value={shift.workedHours} onChange={(event) => changeShiftHour(shift.slotNumber, event.target.value)} /></label>)}<label><span>TC</span><input className="erp-control" type="number" min="0" step="any" value={hourDraft.tcHours} onChange={(event) => changeHour("tcHours", event.target.value)} /></label>{!attendanceLoading && !hourDraft.shifts.length && <span className="erp-matrix-batch-empty-hours">Chưa có ca chấm công; hãy cấu hình bộ ca trước.</span>}</>}
