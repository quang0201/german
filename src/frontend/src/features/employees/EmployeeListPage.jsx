...
export function EmployeeListPage() {
  ...
  const load = () => { setLoading(true); Promise.all([api.get("/api/employees"), api.get("/api/shift-templates")]).then(([employeeRows, shifts]) => { setRows(employeeRows); setShiftTemplates(shifts); }).catch((e) => setError(mapProductionEntryError(e, "Không thể tải nhân viên và bộ ca."))).finally(() => setLoading(false)); };
  useEffect(load, []);
  async function submitCreate(payload) { setCreateSaving(true); setCreateError(""); try { await api.post("/api/employees", payload); setCreateOpen(false); await load(); } catch (e) { setCreateError(mapProductionEntryError(e, "Không thể tạo nhân viên và gán bộ ca.")); } finally { setCreateSaving(false); } }
