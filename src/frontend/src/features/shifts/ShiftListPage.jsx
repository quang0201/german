...
export function ShiftListPage() {
  ...
  return <div className="erp-feature-page"><PageHeader title="Ca làm việc" description="Quản lý bộ ca và khung giờ HC." />{error && <Alert variant="error" title="Không thể hoàn tất thao tác.">{error}</Alert>}<FormSection title="Tạo bộ ca"><Field label="Tên bộ ca" required><input className="erp-control" required value={name} onChange={(e) => setName(e.target.value)} /></Field><div className="erp-form-actions"><button className="erp-button erp-button-primary" type="submit" form="shift-create" disabled={saving}>{saving ? "Đang lưu..." : "Tạo bộ ca"}</button></div></FormSection><form id="shift-create" onSubmit={submit} hidden /><DataTable columns={columns} rows={rows} loading={false} error={error} emptyMessage="Chưa có bộ ca." rowKey="id" /></div>;
}
